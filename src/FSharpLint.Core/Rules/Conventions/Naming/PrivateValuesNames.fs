...
let private getValueOrFunctionIdents typeChecker isPrivate pattern =
...
let private getIdentifiers (args:AstNodeRuleParams) =
    ...
    | AstNode.Binding(SynBinding(access, _, _, _, attributes, _, valData, pattern, _, _, _, _)) ->
        if not (isLiteral attributes) then
            ...
            | Value | Function ->
                let isPrivate = isPrivate args.SyntaxArray args.NodeIndex
                getPatternIdents isPrivate (getValueOrFunctionIdents args.CheckInfo) true pattern
            | _ -> Array.empty
        else
            Array.empty
